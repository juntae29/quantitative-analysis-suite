...
class DataProcessor:
    def load_file(self, file):
        ...
        try:
            if file.name.lower().endswith('.pdf'):
                ...
                # [초경량 저메모리 스트리밍 패치] 판다스 로딩 없이 엑셀 내부 XML 텍스트만 다이렉트 추출 (메모리 5MB 이하 소요)
                try:
                    text_pieces = []
            ...
            # 기존 원본의 텍스트 합성 방식 및 컬럼 형태 완벽 보존
    ...
    def normalize(self, text):
        # 기존 단어 정제 로직 및 사용자가 정의한 불용어(noise_words) 목록 100% 보존
